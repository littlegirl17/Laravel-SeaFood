                ...
                <div class="row" >
                    <div class="col-md-9 col-12 pt-5" style="background-color: #fff;">
                        <div class="row ">
                            <div class="col-sm-5 col-12">
                                <div class="detailImg">
                                    <img src="{{ asset('img/seafood/'.$detail->image) }}" class="img-fluid" id="image_box">
                                </div>
                                <div class="row">
                                ...
                                <div class="d-flex align-items-center">
                                    <p class="card-text">
                                        @if (isset($detail->discount_price))
                                            <h2 class="priceDetail ">{{number_format($detail->discount_price, 0, ',', '.').'đ'}}</h2><h4 class="text-decoration-line-through priceSaleDetail ps-3">{{number_format($detail->price, 0, ',', '.').'đ'}}</h4>
                                        @else
                                        <h2 class="priceDetail ">{{number_format($detail->price, 0, ',', '.').'đ'}}</h2>
                                        @endif
                                    </p>
                                </div>
                                    ...
                                    <div class="input-groupDetail pt-5">
                                        <span class="input-group-btn">
                                            <button type="button" class="btn btn-default btn-number" ><a href="" class="text-decoration-none text-black">-</a></button>
                                        </span>
                                        <input type="text" class="input-number border" value="1" min="1" max="10">
                                        <span class="input-group-btn">
                                            <button type="button" class="btn btn-default btn-number" ><a href="" class="text-decoration-none text-black">+</a></button>
                                        </span>
                                    </div>
                    ...
                    <div class="col-md-3 col-12" >
                        <ul class="list-group " style="border: 1px solid #f0efef;">
                            <li class="list-group-item bg-success active border-0" aria-current="true">Cam kết bán hàng</li>
                            <li class="list-group-item border-0 "><i class="fa-solid fa-tractor pe-3"></i>Hàng từ nông sản nhà vườn</li>
                            <li class="list-group-item border-0"><i class="fa-solid fa-fish-fins pe-3"></i>Hàng từ khai thác thủy hải sản</li>
                            <li class="list-group-item border-0"><i class="fa-solid fa-truck pe-3"></i>Free ship toàn quốc đơn hàng trên 400k</li>
                            <li class="list-group-item border-0"><i class="fa-solid fa-globe pe-3"></i>International shipping not yet available!</li>
                        </ul>
                    </div>
                </div>
            </div>
                            ...
                            <div class="cardhover">
                                <a href="/detail/{{$item->slug}}" class="text-black text-decoration-none">
                                    <div class="card rounded-0 border-0 cardhover2">
                                        <img src="{{ asset('img/seafood/'.$item->image) }}" class="card-img-top" alt="...">
                                    </div>
                                    <h5 class="card-title">{{$item->name}}</h5>
                                    <p class="card-text py-1">
                                        @if (isset($item->discount_price))
                                            <span class="price">{{number_format($item->discount_price, 0, ',', '.').'đ'}}</span><span class="text-decoration-line-through priceSaleDetail ps-3">{{number_format($item->price, 0, ',', '.').'đ'}}</span>
                                        @else
...
<script>
    function clickChangeImg(element){
        let imageItem = element.getAttribute('src'); // tham số element nhận tham chiếu tới đối tượng <img> đã được click. // lấy giá trị của thuộc tính src của hình ảnh đã click
        document.getElementById('image_box').setAttribute('src',imageItem); //đặt giá trị của thuộc tính src của một thẻ hình ảnh khác có id là image_box
    }
</script>
