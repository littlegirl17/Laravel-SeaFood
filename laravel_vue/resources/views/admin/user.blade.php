        ...
        <form id="submitFormAdmin">
            ...
            <div class="buttonProductForm mt-3">
                <button class="btn btnF1">
                    <a href="{{ route('userAdd') }}" class="text-decoration-none text-light"><i
                            class="pe-2 fa-solid fa-plus" style="color: #ffffff;"></i>Tạo mới khách hàng</a>
                </button>
                <button class="btn btnF2" type="button"
                    onclick="submitForm('{{ route('admin.checkboxDeleteUser') }}','post')"><i class="pe-2 fa-solid fa-trash"
                ...
                <table class="table table-bordered pt-3">
                    ...
                    <tbody class="table-body">
                        @foreach ($users as $item)
                            <tr class="">
                                <td class=""><img src="{{ asset('uploads/' . $item->image) }}" alt=""
                                        style="width: 80px; height: 80px; object-fit: cover;"></td>
                                <td class="">{{ $item->email }}</td>
                                <td>{{ $item->userGroup->name ?? 'Mặc định' }}</td>
                                ...
                                <td class="m-0 p-0">
                                    <div class="actionAdminProduct m-0 py-3">
                                        <button class="btnActionProductAdmin2"><a href="{{ route('userEdit', $item->id) }}"
                                                class="text-decoration-none text-light"><i class="pe-2 fa-solid fa-pen"
                                                    style="color: #ffffff;"></i>Chỉnh sửa tài khoản</a></button>
                                    </div>
                                </td>
                            </tr>
                        @endforeach
